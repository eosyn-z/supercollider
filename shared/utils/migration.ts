/**
 * Migration utilities for upgrading existing Supercollider installations
...
import { DEFAULT_WORKFLOW_EXECUTION_CONFIG, DEFAULT_INJECTION_CONFIG } from '../config/schema';

export class SupercolliderMigration implements MigrationUtility {
  private version = '1.0.0';

  /**
   * Convert legacy workflow to enhanced workflow with new features
   */
  convertLegacyWorkflow(workflow: Workflow): EnhancedWorkflow {
    const enhanced: EnhancedWorkflow = {
      ...workflow,
  ...
  /**
   * Backfill agent tags based on existing capabilities and names
   */
  ...
  /**
   * Migrate old execution state to enhanced execution state
   */
  ...
  /**
   * Generate batch groups from existing subtasks based on dependencies
   */
  private generateBatchGroups(workflow: Workflow): BatchGroup[] {
    ...
    for (const subtask of workflow.subtasks) {
      ...
      // Find subtasks that can run in parallel with this one
      const parallelSubtasks = workflow.subtasks.filter(s => {
        if (processedSubtasks.has(s.id)) return false;
        
        // Check if this subtask has any blocking dependencies on unprocessed subtasks
        const hasBlockingDeps = s.dependencies?.some(dep => 
      ...
      parallelSubtasks.forEach(subtask => {
        const batchableSubtask = {
          ...
          injectedContext: subtask.description // Will be enhanced by context injector
        };
  ...
  /**
   * Infer agent tags based on existing data
   */
  private inferAgentTags(agent: Agent): AgentTag[] {
    const tags: AgentTag[] = [];
    ...
    if (agent.capabilities) {
      agent.capabilities.forEach(capability => {
        ...
        if (capName.includes('creation') || capName.includes('generation')) {
          if (!tags.includes('CREATION')) tags.push('CREATION');
        }
        if (capName.includes('research') || capName.includes('search')) {
          if (!tags.includes('RESEARCH')) tags.push('RESEARCH');
        }
        ...
        if (capName.includes('code') || capName.includes('programming')) {
          if (!tags.includes('CODEGEN')) tags.push('CODEGEN');
        }
      });
    }

    // Default to CREATION if no tags found
    if (tags.length === 0) {
      tags.push('CREATION');
    }
   ...
   */
  private mapLegacyStatus(oldStatus: string): EnhancedExecutionState['status'] {
    const statusMap: Record<string, EnhancedExecutionState['status']> = {
      'DRAFT': 'pending',
      'PLANNING': 'pending',
      'EXECUTING': 'running',
      'RUNNING': 'running',
  ...
  /**
   * Calculate progress from old state
   */
  private calculateProgress(oldState: any) {
  ...
  /**
   * Migrate old batch format to new format
   */
  private migrateBatches(oldBatches: any[] = []) {
    return oldBatches.map(batch => ({
      batchId: batch.id || batch.batchId || `migrated-${Date.now()}`,
      subtaskIds: batch.subtasks?.map((s: any) => s.id) || batch.subtaskIds || [],
      status: this.mapLegacyStatus(batch.status) as any,
  ...
  /**
   * Migrate old subtask executions to new format
   */
  private migrateSubtaskExecutions(oldExecutions: any = {}) {
    const executions: Record<string, any> = {};
    
    Object.entries(oldExecutions).forEach(([subtaskId, execution]: [string, any]) => {
  ...
  /**
   * Generate timeline from old state
   */
  private generateTimeline(oldState: any) {
    const timeline = [];
   ...
   */
  private estimateBatchTime(subtasks: any[]): number {
    if (subtasks.length === 0) return 0;
    
    // For parallel execution, time is roughly the longest subtask
...
`;
  ...
  static validateMigration(
    ...
    migratedWorkflows: EnhancedWorkflow[]
