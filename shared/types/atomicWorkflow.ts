/**
 * Type definitions for atomic workflow decomposition system
 */
...
export type AtomicTaskType = 
  ...
  | 'generate_code' | 'generate_data' | 'generate_document' | 'generate_presentation'
  
  // Editing/Processing tasks
  | 'edit_audio' | 'edit_video' | 'edit_image' | 'edit_text' | 'edit_code'
  | 'process_data' | 'process_document' | 'process_media'
  
  // Analysis tasks
  | 'analyze_content' | 'analyze_data' | 'analyze_sentiment' | 'analyze_structure'
  | 'extract_data' | 'extract_features' | 'extract_metadata'
  
  // Transformation tasks
  | 'convert_format' | 'merge_media' | 'split_content' | 'resize_media'
  | 'compress_file' | 'optimize_file' | 'enhance_quality'
  
  // Output/Delivery tasks
  | 'validate_output' | 'display_result' | 'save_result' | 'export_data'
  | 'send_notification' | 'create_archive' | 'publish_content'
...
export interface TaskInput {
  ...
  dataType: string; // specific type like 'string', 'number', 'image/jpeg', etc.
  required: boolean;
...
export interface TaskOutput {
  ...
  size?: { width?: number; height?: number; duration?: number; fileSize?: number };
}
...
export interface DecompositionRule {
  ...
  name: string;
  description: string;
  trigger: string; // Natural language pattern or intent
  pattern: RegExp;
...
export interface TaskDependency {
  ...
  delay?: number; // Delay before starting dependent task
}
...
export interface ExecutionEdge {
  ...
  condition?: string;
  weight: number; // For critical path calculation
}
...
export interface TaskInputMapping {
  ...
  sourceId?: string; // Task ID or file ID
  sourceOutputId?: string;
...
export interface WorkflowMetadata {
  ...
  compatibleFiles: string[]; // MIME types
  successRate: number;
