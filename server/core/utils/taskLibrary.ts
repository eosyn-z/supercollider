/**
 * Task Library - Predefined atomic task templates for common operations
 * Provides a comprehensive collection of reusable atomic tasks
 */
...
export class TaskLibrary {
  private static taskTemplates: Map<string, AtomicTask> = new Map();
  private static customTemplates: Map<string, TaskTemplate> = new Map();
  ...
  /**
   * Create custom task from template
   */
  ...
  static validateTaskCompatibility(task1: AtomicTask, task2: AtomicTask): boolean {
    // Check if task2 can receive output from task1
  ...
  /**
   * Get task template for customization
   */
  ...
  static getAvailableTaskTypes(): AtomicTaskType[] {
    const types = new Set<AtomicTaskType>();
    for (const task of this.taskTemplates.values()) {
      types.add(task.type);
    }
  ...
  private static initializeGenerationTasks(): void {
  ...
  private static initializeProcessingTasks(): void {
  ...
  private static initializeAnalysisTasks(): void {
    // Content Analysis
    this.addTask({
  ...
  private static initializeTransformationTasks(): void {
    // Format Conversion
    this.addTask({
  ...
  private static initializeCoordinationTasks(): void {
    // Task Coordination
    this.addTask({
  ...
  private static initializeOutputTasks(): void {
    // Output Validation
    this.addTask({
  ...
  private static initializeCustomTemplates(): void {
  ...
  private static addTask(task: AtomicTask): void {
    this.taskTemplates.set(task.id, task);
  }

  private static cloneTask(task: AtomicTask): AtomicTask {
    return JSON.parse(JSON.stringify(task));
  }

  private static matchesCriteria(task: AtomicTask, criteria: TaskSearchCriteria): boolean {
    if (criteria.type && task.type !== criteria.type) return false;
    if (criteria.complexity && task.complexity !== criteria.complexity) return false;
    if (criteria.maxDuration && task.estimatedDuration > criteria.maxDuration) return false;
    ...
    if (criteria.inputTypes && criteria.inputTypes.length > 0) {
      const hasCompatibleInputs = criteria.inputTypes.some(inputType =>
        task.inputs.some(input => input.dataType === inputType)
      );
      if (!hasCompatibleInputs) return false;
    }

    if (criteria.outputTypes && criteria.outputTypes.length > 0) {
      const hasCompatibleOutputs = criteria.outputTypes.some(outputType =>
        task.outputs.some(output => output.dataType === outputType)
      );
      if (!hasCompatibleOutputs) return false;
    }

    if (criteria.textSearch) {
      const searchText = criteria.textSearch.toLowerCase();
      const taskText = `${task.name} ${task.description}`.toLowerCase();
      if (!taskText.includes(searchText)) return false;
    }

    return true;
  }

  private static sortSearchResults(results: AtomicTask[], criteria: TaskSearchCriteria): AtomicTask[] {
    return results.sort((a, b) => {
      // Sort by relevance, complexity, and estimated duration
      let scoreA = a.priority;
      let scoreB = b.priority;

      // Boost score for exact type matches
      if (criteria.type === a.type) scoreA += 10;
      if (criteria.type === b.type) scoreB += 10;

      // Prefer simpler tasks if no complexity preference
  ...
  private static areCompatibleTypes(outputType: string, inputType: string): boolean {
    // Direct match
  ...
  private static getDefaultRetryPolicy(): any {
    return {
      maxRetries: 3,
      backoffStrategy: 'exponential',
      baseDelay: 2000,
  ...
  private static getDefaultValidation(): any {
    return {
      inputValidation: [],
      outputValidation: [],
      businessRules: [],
  ...
  private static getCategoryForTask(task: AtomicTask): string {
